...
describe('the ticket registration endpoint', () => {
	...
	// Paid events used to be refused here and sent to a Paystack checkout.
	// There is no checkout any more: the foundation collects the money by EFT
	// or in cash at the event, so a priced event registers exactly like a free
	// one and the price rides along on the ticket for the attendee's reference.
	it('registers a ticketed event, since payment now happens off the platform', async () => {
		mockAttempt({ event: eventFields({ isTicketed: true, price: 150 }), result: 'committed' });
		const response = await post(registerBody());
		expect(response.status).toBe(200);
	});
